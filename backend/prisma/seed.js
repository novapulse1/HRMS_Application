...
async function main() {
    ...
    if (!superAdminPassword) {
        superAdminPassword = crypto_1.default.randomBytes(12).toString('base64url');
    }
    const superAdminPasswordHash = await bcryptjs_1.default.hash(superAdminPassword, 10);
    let superAdmin;
    ...
    if (process.env.NODE_ENV !== 'production') {
        ...
        const demoAdminPassword = process.env.DEMO_ADMIN_INITIAL_PASSWORD ||
            crypto_1.default.randomBytes(10).toString('base64url');
        const demoAdminPasswordHash = await bcryptjs_1.default.hash(demoAdminPassword, 10);
        ...
        if (!demoCompany) {
            demoCompany = await prisma.company.create({
                data: {
                    ...
                    license_plan: client_1.LicensePlan.pro,
                    license_status: client_1.LicenseStatus.active,
            ...
            const adminEmployee = await prisma.employee.create({
                data: {
                    ...
                    employment_type: client_1.EmploymentType.full_time,
            ...
            await prisma.user.create({
                data: {
                    ...
                    role: client_1.TenantRole.company_admin,
                    must_change_password: true,
                    is_active: true,
                },
            });
